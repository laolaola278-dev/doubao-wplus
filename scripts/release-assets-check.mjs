...
// `new URL('..', import.meta.url).pathname` yields a doubled drive prefix on Windows.
...
const extensionZips = ['chrome', 'edge', 'firefox'].map((browser) => ({
  browser,
  path: resolve(distDir, `doubao-wplus-${version}-${browser}.zip`),
}));
const sourceZip = resolve(distDir, `doubao-wplus-${version}-sources.zip`);
